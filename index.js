...
const colour = (r, g, b) => [boundColour(r), boundColour(g), boundColour(b)];
...
const sdfSphere = (p, x, y, z, r) => lengthV3([p[0] - x, p[1] - y, p[2] - z]) - r;
...
const getDist = (p) => {
  ...
  distances.push(sdfSphere(p, 2, 0.25 + Math.abs(Math.sin(t + 1) * 0.5), 4, 0.25));
  distances.push(sdfSphere(p, 2.5, 0.25 + Math.abs(Math.sin(t + 2) * 0.5), 4, 0.25));
  distances.push(sdfSphere(p, 3, 0.25 + Math.abs(Math.sin(t + 3) * 0.5), 4, 0.25));
  distances.push(sdfSphere(p, 3.5, 0.25 + Math.abs(Math.sin(t + 4) * 0.5), 4, 0.25));
...
const castRay = (origin, dir) => {
  ...
  for (let i = 0; i < 128; i++) {
    const p = [origin[0] + dist * dir[0], origin[1] + dist * dir[1], origin[2] + dist * dir[2]];
    debug('p=%o', p);
...
const getLight = (p) => {
  ...
  const lightDirection = normalise([lightPos[0] - p[0], lightPos[1] - p[1], lightPos[2] - p[2]]);
  ...
  const pu = [p[0] + 0.02 * hitNormal[0], p[1] + 0.02 * hitNormal[1], p[2] + 0.02 * hitNormal[2]];
...
const shade = (u, v) => {
  ...
  const p = [cameraOrigin[0] + len[0], cameraOrigin[1] + len[1], cameraOrigin[2] + len[2]];
...
while (t < 7) {
  ...
  for (let y = height; y >= 0; y -= 2) {
    for (let x = 0; x < width; x++) {
      let u = (x - .5 * width) / height;
      let v2 = (y - .5 * height) / height;
      let v1 = ((y + 1) - .5 * height) / height;
